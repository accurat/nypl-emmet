'use strict';
angular.module('ngScrollbar', []).directive('ngScrollbar', ['$parse', '$window', function($parse, $window) {
	return {
		restrict : 'A',
		replace : true,
		transclude : true,
		link : function(scope, element, attrs) {
			...
			var calcStyles = function() {
				scrollboxStyle = {position : 'relative', overflow : 'hidden', 'max-width' : '100%', height : '100%'};
				if (page.height) scrollboxStyle.height = page.height + 'px';
				draggerStyle = {position : 'absolute', height : dragger.height + 'px', top : dragger.top + 'px'};
				draggerLineStyle = {position : 'relative', 'line-height' : dragger.height + 'px'};
				pageStyle = {position : 'relative', top : page.top + 'px', overflow : 'hidden'};
			};
			...
			var buildScrollbar = function() {
				...
				if (page.height < page.scrollHeight) 
				{
					...
					scope.containerStyle = {'padding-right': '15px'};
				...
				else scope.showYScrollbar = false;
			};
			...
			if (!!attrs.rebuildOn) attrs.rebuildOn.split(' ').forEach(function(eventName) {scope.$on(eventName,rebuild);});
			
			if (attrs.hasOwnProperty('rebuildOnResize')) win.on('resize', rebuild);
		},
		template : function() { return '<div>'
				+ '<div class="ngsb-wrap">'
				+ '<div class="ngsb-container" ng-style="containerStyle" ng-transclude></div>'
